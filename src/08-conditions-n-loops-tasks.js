...
function isTriangle(a, b, c) {
  ...
  for (let i = 0; i < sides.length; i += 1) {
    const sumOfOthers = sides.reduce((sum, side, index) => (index !== i ? sum + side : sum), 0);
    if (sides[i] >= sumOfOthers) {
      return false;
    }
  }
  return true;
...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const commonX = xCoords.filter((el) => xCoords.indexOf(el) !== xCoords.lastIndexOf(el));
  const commonY = yCoords.filter((el) => yCoords.indexOf(el) !== yCoords.lastIndexOf(el));
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (openBrackets.indexOf(str[i]) !== -1) {
      stack.push(str[i]);
    } else if (Object.values(brackets).indexOf(str[i]) !== -1) {
...
function getCommonDirectoryPath(pathes) {
  ...
  // Find the common prefix of all path arrays
  let commonPath = splitPaths[0];

  for (let i = 1; i < splitPaths.length; i += 1) {
    let j = 0;
    while (j < commonPath.length && j < splitPaths[i].length
      && commonPath[j] === splitPaths[i][j]) {
      j += 1;
    }
    commonPath = commonPath.slice(0, j);
  }

  // Join the common path elements with '/' and add a trailing '/' if necessary
